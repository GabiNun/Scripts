...
iwr 'raw.githubusercontent.com/ionuttbara/windows-defender-remover/main/Remove_Defender/RemoveDefender.reg' -OutFile "$env:TEMP\RemoveDefender.reg";iwr 'github.com/ionuttbara/windows-defender-remover/raw/main/Remove_SecurityComp/Remove_SecurityComp.reg' -OutFile "$env:TEMP\Remove_SecurityComp.reg"
'RemoveDefender','Remove_SecurityComp'|%{iwr "https://github.com/ionuttbara/windows-defender-remover/raw/main/$($_ -replace '_','/')/$_.reg" -OutFile "$env:TEMP\$_.reg"}
...
Register-ScheduledTask -TaskName r -Action (New-ScheduledTaskAction -Execute reg -Argument "import $env:temp\Remove_SecurityComp.reg");$s=New-Object -ComObject Schedule.Service;$s.Connect();$s.GetFolder('\').GetTask('r').RunEx($null,0,0,'NT SERVICE\TrustedInstaller');Unregister-ScheduledTask -TaskName r -Confirm:$false
