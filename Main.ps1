...
$store='HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Appx\AppxAllUserStore'; $appx=Get-AppxPackage -AllUsers -Name "Microsoft.SecHealthUI"; $extraSids = if (Test-Path $store) { Get-ChildItem $store -ea 0 | ForEach-Object { $_.PSChildName } | Where-Object { $_ -like 'S-1-5-21*' } } else { @() }; foreach ($sid in @('S-1-5-18') + $extraSids) { New-Item "$store\EndOfLife\$sid\$($appx.PackageFullName)" -Fo | Out-Null }; New-Item "$store\Deprovisioned\$($appx.PackageFamilyName)" -Fo | Out-Null; Remove-AppxPackage -AllUsers -Package $appx.PackageFullName -ea SilentlyContinue
