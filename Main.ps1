...
$a=New-ScheduledTaskAction -Execute reg.exe -Argument "import `"$env:TEMP\Defender.reg`"";Register-ScheduledTask -TaskName TestTask -Action $a | Out-Null;$s=New-Object -ComObject Schedule.Service;$s.Connect();$f=$s.GetFolder('\');$t=$f.GetTask('TestTask');$t.RunEx($null,0,0,'NT SERVICE\TrustedInstaller') | Out-Null;$f.DeleteTask('TestTask',0)
